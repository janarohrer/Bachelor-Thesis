...
folder = 'thesis_data/TFI_ABD_PEL_EXTR_15X15X15_TRA_LR500XAP449XHF432_0002';
...
iso = transpose(-round(info_mr{1}.ImagePositionPatient+[0;24;47]));
%resize voxels to be 1mm x 1mm x 1mm

slices_mr = imresize3(image_mr,[dimx,dimy,dimz]);
slices_mr = slices_mr(25:284,:,48:312); %to generalize,insert option to set these parameters

%import processed mr data
mr_wd = load('processed_mr/mr_wd_15x15.mat');
mr_wd = mr_wd.mr_wd;
mr_imd = load('processed_mr/mr_imd_15x15.mat');
...
% end
% % mr_wd = imadjustn(mr_wd, [0.08 0.8]);
...
pad = 3 + min([mod(z_shift,20) mod(-z_shift,20)]);
figure;
...
montage(im_mount/M)
%
central_z = input('enter the index of the middle slice (most prominent grid): ')+central_shift - pad ;
...
cpselect(circ,slices_mr(:,:,central_z)/max(max(slices_mr(:,:,central_z)))); 
